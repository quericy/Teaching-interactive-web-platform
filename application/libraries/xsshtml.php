...
class XssHtml
{
    ...
    private $m_xss;
    private $m_ok;
    private $m_AllowAttr = array('title', 'src', 'href', 'id', 'class', 'style', 'width', 'height', 'alt', 'target', 'align','text-align');
    private $m_AllowTag = array('a', 'img', 'br', 'strong', 'b', 'code', 'pre', 'p', 'div', 'em', 'span', 'h1', 'h2', 'h3', 'h4', 'h5', 'h6', 'table', 'ul', 'ol', 'tr', 'th', 'td', 'hr', 'li', 'u');

    /**
     * 构造函数(改)
     * @param array $params 适用于ci框架的初始化参数数组($html 待过滤的文本,$charset 文本编码，默认utf-8,$AllowTag 允许的标签，如果不清楚请保持默认，默认已涵盖大部分功能，不要增加危险标签)
     */
    public function __construct($params)
    {
        $html = $params['html'];
        $charset =isset($params['charset'])?$params['charset']:null;
        $AllowTag =isset($params['AllowTag'])? $params['AllowTag']:null;
